...
\begin{equation}
area([a.b])=\int \limits _{a}^{b} \kappa(t)-\iota_{T}(t) \mathrm{d}t
\end{equation}
\iffalse
\subsubsection{Algorithm Version}
...
\begin{equation}
T = \inf_{t \in U} \lvert \frac{\mathrm{d}}{\mathrm{d}t}\kappa(t) \rvert
\end{equation}
The criteria, whether a segment is within a chart is:
\begin{equation}
[a,b] \subseteq U_{m} \Rightarrow \exists  \frac{\mathrm{d}}{\mathrm{d}t}\kappa(t)>0 \land \nexists \frac{\mathrm{d}}{\mathrm{d}t}\kappa(t)\leq0
\end{equation}
...
\begin{align}
\kappa'_{y}(a), \kappa'_{y}(b) = 0; \text{(Optional?)}
\end{align}
...
\subsubsection{Cliff}
A maximal curvature $\frac{(\pi r^2) /2}{2r}$ is defined by a unit half circle $r=1$. So curvature is no more than $(x')^2 + (y')^2 =1$  under any linear transformation of any segment.
