...
In order to compare the length from $[a,b] \subseteq A$ with $[a.b] \subseteq X$, we have to connect all $P_{n}, P_{n+1} \in X$ with linear bezier splines, because $X$ is discrete:
\begin{align}
lines: X \times X \rightarrow B\\
c_{1}=\{P_{n},\frac{P_{n}+P_{n+1}}{2},P_{n+1}\} \mapsto \gamma_{c_{1}}: \text{Straight line}\\
