...
\end{align*}
and is composed of four quadratic bezier splines $\gamma_{c}(t)$ with control points $\{c_{1},...,c_{4}\}$ per chart of M:
\begin{align*}
...
(c_{1},...,c_{4}) \mapsto \gamma_{c_{1}} \cup \gamma_{c_{2}} \cup \gamma_{c_{3}} \cup \gamma_{c_{4}}
\end{align}
\begin{align}
\gamma_{c}: \mathbb{R} \rightarrow \mathbb{R}^2; t \mapsto (x,y)
\end{align}
...
c_{1}=\{P_{n},P_{n}+\frac{(P_{n+1}-P_{n})}{2},P_{n+1}\} \mapsto \gamma_{c_{1}}: \text{Straight line}\\
...
Auxiliary functions are functions that can be applied to any interval $[a,b] \subseteq B,A$ with a constraint.
...
\end{equation}
A threshold can be chosen, which makes the segment straight or curved.\\
It is closely related to the cliff measure

\subsubsection{Cliff Divider}
