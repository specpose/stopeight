...
The purpose of the Analyzer is to find intervals in a Vector Graph. The input Vector Graph may be obtained from the Grapher, or it may be present in another mathematical context. Certain properties, such as the maximal angle or the number of measure spaces involved, are inherited from this context.\\\\
...
C_{4} \ni b'
\end{align}
Note: Curvature is no more than $(x')^2 + (y')^2 =1$  under any linear transformation of any segment. (See Separation / Measure Space)
\iffalse
...
\subsection{Straight}
A Straight is a segment $[a,b]$ where $straightness(a,b)$ is smaller than the straightness of the unit circle (or straightness of exp?). It is a part of a Spike, or transfering the Straight half of a Dune to an adjacent ZigZag, or being used as part of Dune detection. It implies a reduction of measures which are smaller than the threshold (!). A Corner is $inserted$ .
\begin{align}
Max_{Straight}=\\
C_{3} \ni \frac{b-a}{2}
\end{align}
...
\begin{align}
jitter_{T}(a.b)=\int \limits _{a}^{b}\lvert \iota_{X}(t) - \iota_{T}(t) \rvert \mathrm{d}t\\
area_{vert,T}(a,b)=\int \limits _{a}^{b} \lvert \xi_{T}(t)-\iota_{T}(t) \rvert \mathrm{d}t
\end{align}
\begin{align}
straightness_{Abs,T}(a.b)=\frac{jitter_{T}(a,b)}{diameter_{horiz,T}(a,b)}
\end{align}
\begin{align}
curvature_{Abs,T}(a,b) = \frac{area_{vert,T}(a,b)}{diameter_{horiz,T}(a,b)}
\end{align}
