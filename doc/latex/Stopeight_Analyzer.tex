...
\begin{equation}
area([a.b])=\int \limits _{a}^{b} \lvert \xi(t)-\iota_{T}(t) \rvert \mathrm{d}t
\end{equation}
\iffalse
\subsubsection{Algorithm Version}
...
\begin{equation}
diameter([a,b])=\int \limits _{a}^{b} \lvert \iota_{T}(b)-\iota_{T}(a) \rvert \mathrm{d}t
\end{equation}
...
\end{align}
\begin{align}
visibility([a,b])=\frac{(b-a)}{\lvert b-a \rvert}curvature[a,b]
\end{align}
...
\norm{z} = \sqrt{isolation([a,b])^2+visibility([a,b])^2}
\end{align}
