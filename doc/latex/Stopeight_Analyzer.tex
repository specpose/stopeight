...
\subsubsection{Area}
The integral of the area under the transformed arc curve function $transform(\kappa) = \kappa'$, so that: (partial derivative?)
\begin{align}
y(\kappa'(a)), y(\kappa'(b)) = 0\\
area([a,b])= \int \limits _{x(\kappa'(a))}^{x(\kappa'(b))} y(\kappa'(t))\partial_{x} \kappa'(t)
\end{align}

\subsubsection{Curvature}
Curvature is determined by a ratio of Area to Diameter.
\begin{equation}
curvature([a,b]) = \frac{area([a,b])}{\sqrt{x(\kappa(b) - \kappa(a))^2+y(\kappa(b) - \kappa(a))^2}}
\end{equation}
...
\end{bmatrix}
\end{equation}
The composite $transform(\kappa)$ is based on parameter $t$ and $h_{1} \in S \subset U_{m};h_{2} \in C \subset U_{m}; h_{3} \in E \subset U_{m}$ and.
