...
\subsection{Straight}
A Straight is a segment $[a,b]$ where $straightness(a,b)$ is smaller or equal to the ratio of the corresponding arc-length of $2n$ regular polygon sides in a circle to the length of the direct line $\mathcal{AC}$. Choosing the polygon $n\geq 4$ is subject to the underlying pen-tablet or signal analog to digital converter. It is a part of an Edge,
\begin{align}
Max_{Straight}=\frac{4\pi r / n}{\mathcal{AC}}
\end{align}
...
\end{equation}
(both vice-versa)\\\\
A special case is where the whole chart is flat and Straight. The Corner is $moved$ (~\cite{Analyzer}[3.23]).
\begin{equation}
...
A Moustache is a CC-Separator. It is handled by Cliff or Crest detection, but it would only be a $T_{4}$ if not caught by Compact Covers. Indicates a break or change of direction $dir_{Z}$ in Swells and Spirals $[a,b',c]$ at the most.
\begin{equation}
b' \in T_{\infty}
\end{equation}
...
\subsubsection{Spike}
A Spike is a TT-Qualifier. If it is not in the in the Vector Graph $X$, it has to be approximated in the transversality $Y$. It is of highest computational priority.\\\\
...
\subsubsection{Ramp}
A Ramp is a CC-Qualifier. A frontside Ramp is handled by Edge detection, a backside Ramp is handled by Crest detection. It is not a break of the Compact Cover and is only a $T_{4}$.
\begin{equation}
