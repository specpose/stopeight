...
A single Cliff can have no more than the maximum curvature. Is a part of a Spiral.
\begin{equation}
Max_{Curve}=
\end{equation}
...
\subsection{Straight}
A Straight is a segment $[a,b]$ where $straightness(a,b)$ is smaller than the straightness of the unit circle (or curvature of exp?). It is a part of a Spike, or transfering the Straight half of a Dune to an adjacent ZigZag, or being used as part of Dune detection. It implies a reduction of measures which are smaller than the threshold. A Corner is $inserted$ at $\frac{b-a}{2}$.
\begin{equation}
Max_{Straight}=
\end{equation}
...
\subsection{Dune}
A backside Dune $[a,b]$ is a curved segment $[a,b']$, which is followed by a maximal Straight section $[b',b]$.
\begin{equation}
straightness(a,b')>Max_{Straight};straightness(b',b)\leq Max_{Straight}
\end{equation}
It is only used as an $insertion$ section, or if no other entities are being found. It occurs in the lowest priority of analysis (i.e. as a section of a whole Turn, where a=S and b=E) or as forward/backward parts of a Spike.\\

\subsection{Spike}
A backside Spike $[a,b']$ is an iteration of:
\begin{enumerate}
\item A backside Dune $[a,a']$
\item A backside Dune $[b,b']$, where $a' \leq b'$ 
\end{enumerate}
