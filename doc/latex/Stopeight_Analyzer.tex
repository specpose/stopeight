...
(Does it exist?)
\begin{equation}
T = \inf_{t \in U} \lvert \frac{\mathrm{d}}{\mathrm{d}t}\gamma(t) \rvert \approx 0
\end{equation}
...
\subsection{Swing}
A Swing is the segment $a,b \in Y$, where a legal segment is followed by a sign change $[c,d]; c<d$ under affine transformation $\xi'$. The criteria, whether a segment is within a chart is ~\cite[\nopp 20.7]{Loring}:
\begin{equation}
...
\begin{equation}
T = \inf_{t \in U} \lvert \frac{\mathrm{d}^2}{\mathrm{d}^2t}\gamma(t) \rvert \approx 0
\end{equation}
...
\subsection{Dune}
(half Dune is Cliff)
They only occur in last priority or as forward/backward parts of a Spike.

\subsection{Spike}
A forward Dune produces two Straights if it overlaps with a backward Dune by a certain margin.\\
...
The sign of times.
\begin{equation}
s_{const} = \sum_{\mu_{+}}^{}o -  \sum_{\mu_{-}}^{}o
\end{equation}
...
\begin{equation}
isolation(a,b)=s \prod straightness_{Abs}(a,b)
\end{equation}
