...
Appendix: A pseudo inverse can reconstruct the signal from the approximation, effectively making it a suitable format for compression. The signal would be calculated from the second order total derivative and the sign of the chart. More about this later...
\begin{equation}
...
The curve functions can be interprated as $\gamma(x(t),y(t))$. The derivative in the following is meant to be the total derivative.
\begin{align}
\frac{\mathrm{d}}{\mathrm{d} t} \gamma (t) = \frac{\mathrm{d}}{\mathrm{d} t} \gamma (x(t),y(t)) = \gamma(t) \begin{bmatrix} \frac{\partial}{\partial x}  \mathrm{d} x & \frac{\partial}{\partial y} \mathrm{d} y \end{bmatrix} \frac{1}{\mathrm{d}t}
\end{align}\\
The second order total derivative induces directionality.\\
\begin{align}
\frac{\mathrm{d}^2}{\mathrm{d}^2t}\gamma(t)=\partial^2 \gamma(t) \begin{bmatrix} \frac{\mathrm{d}^2x}{\partial^2 x} & \frac{\mathrm{d}^2y}{\partial^2 y}\end{bmatrix}\frac{1}{\mathrm{d}^2t}
\end{align}
