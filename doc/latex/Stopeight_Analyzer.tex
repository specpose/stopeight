...
Derivatives are opening up interval measure spaces. The maximum order of $derivatives$ is set by the order of recursion. The scale-space depends on the maximal order of the polynomials considered. In the method discussed in this paper, they are quartic with restrictions, i.e. the representation is quadratic. (The functions $q$ and the points $S,C,E$ pose a constraint on the quartic analytic form and its inverse.)\\\\
