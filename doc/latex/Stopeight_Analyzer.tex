...
\subsubsection{Area}
The integral of a transformed curve function $transform(\kappa)$, so that:
\begin{align}
y(transform(\kappa(a)), y(transform(\kappa(b)) = 0\\
\int \limits _{x(transform(\kappa(a)))}^{x(transform((\kappa(b)))} y(transform(\kappa(t)))\mathrm{d}x(transform(\kappa(t)))
\end{align}
