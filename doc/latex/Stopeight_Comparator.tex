...
\begin{align}
isolation(a,b)= \int \limits _{y(a)}^{y(b)} \int \limits _{x(a)}^{x(b)} \lvert \vartheta (\iota_{W} (t))*(\frac{\mathrm{d}}{\mathrm{d}x},\frac{\mathrm{d}}{\mathrm{d}y})\rvert \mathrm{d}x\mathrm{d}y\\
...
\begin{align}
\frac{\log isolation_{\iota_{X}}}{dir_{Y}\log\overline{\vartheta (SVE')}} \odot \frac{visibility_{\xi}}{\lvert \cos\upsilon (VE)+\mathrm{i}\sin\upsilon (VE)\rvert}
\end{align}
