...
The square root of the product $\sqrt{\xi'(t)*\iota'_{X}(t)}$ happens to be the geometric mean. Taking the mean instead of the ratio has a few implications. Lets assume we have computed the Stopeight Analyzer transversality by applying the affine transformations in a positive direction $dir_{Y}=1$. We now measure this against the actual input in the same direction $dir_{X}=1$. The resulting number is entirely real. \\\\
Also it can have values between $[0,\inf]$, which is useful for this convolution:\\
\begin{equation}
...
\chapter{Visibility}
Visibility is computed by creating an auxiliary (not transversality)
\begin{equation}
h: Y \rightarrow X \rightarrow O
\end{equation}
