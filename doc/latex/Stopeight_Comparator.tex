...
\begin{align}
\phi (U_{m}) = \phi (E \in U_{m-1})+\phi(U_{m});\phi (U_{0})=0
\end{align}
...
\begin{align}
cartesian(a,b)=\int \limits _{z(a)}^{z(b)} \vartheta (x+\mathrm{i}y) \mathrm{d} t\\
(\int,\norm{t}=\norm{z''}_1)
\end{align}
...
\begin{align}
z_{n} = \int \limits _{t_{1}}^{t_{2}} \lvert \underbrace{\upsilon \circ waveform_{1}(x(t),y(t))}_{\text{orthogonal }\upsilon} \rvert \mathrm{e}^{-2\pi t/(t_{2}-t_{1})}\mathrm{d}t
\end{align}
...
\begin{align}
\hat{f}(1/\tau) = \int \limits _{-\infty}^{\infty} \lvert \underbrace{\upsilon \circ waveform_{1}(x(t),y(t))}_{\text{orthogonal }\upsilon} \rvert \mathrm{e}^{-2\pi t/\tau}\mathrm{d}t
\end{align}
