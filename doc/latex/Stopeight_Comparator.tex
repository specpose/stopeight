...
\begin{align}
O=\{(x',y') \in \mathbb{R}^2 \vert \min_{U_{m}} \norm{X-O}_{2} \}\\
\norm{X-O}_{2}=\sqrt{(x-x')^2 + (y-y')^2}
\end{align}
\begin{align}
visibility(a,b)= \frac{\int \limits _{a}^{b}\norm{A-O}_{2}* (\cos(angle(t)) + \mathrm{i}\sin(angle(t))) \mathrm{d}t}{\int \limits _{a}^{b}\norm{X-O}_{2}* (\cos(angle(t)) + \mathrm{i}\sin(angle(t))) \mathrm{d}t}\\
angle: t \mapsto \phi
\end{align}
