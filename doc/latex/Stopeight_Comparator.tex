...
A geometric proof.\\\\
Consider the $z_{n}$ of a $waveform$ \cite{Grapher}[3.1] representing two consecutive, perfect half circles facing opposite orientation $ori$ on the $xaxis$, where $t_{1}$ is the start of the first half-circle and $t_{2}$ is the end of the second half-circle.
\begin{align}
z_{n} = \int \limits _{a}^{b}\cos \frac{2\pi}{(t_{2}-t_{1})}t + \sin \frac{2\pi}{(t_{2}-t_{1})}t \mathrm{d}t;a,b\geq 0 \land a,b\leq \pi \land a\leq b\\
z_{n} = \int \limits _{a}^{b}-(\cos \frac{2\pi}{(t_{2}-t_{1})}t + \sin \frac{2\pi}{(t_{2}-t_{1})}t) \mathrm{d}t;a,b\geq \pi \land a,b\leq 2\pi \land a\leq b
\end{align}
Note: The resulting $z_{n}$ is entirely real.\\\\
Factor out the complex radius by dividing with the signal base of the corresponding frequency.
\begin{align}
z_{n} = \int \limits _{t_{1}}^{t_{2}} \lvert \underbrace{\upsilon \circ waveform_{1}(x(t),y(t))}_{\text{orthogonal }\upsilon} \rvert \mathrm{e}^{-\mathrm{i}2\pi t/(t_{2}-t_{1})}\mathrm{d}t;a,b\geq 0 \land a,b\leq 2\pi \land a\leq b\label{eq:4}
\end{align}
Note: Choosing a $waveform$ $bs$ other than 1 is not required, because there is no jitter (perfect).\\
The resulting $phaseprofile_{t_{2}-t_{1}}$ is entirely real!\\\\
$phaseprofile_{m}$ ($m\Rightarrow \text{1 vector}$) is the Fourier transform $\mathrm{e}^{-\mathrm{i}\omega t}$
\begin{align}
z_{0} = \hat{f}(1/T)\circ \underbrace{\lvert \upsilon \circ waveform_{1}(x(t),y(t))\rvert}_{\text{orthogonal }\upsilon} = \int \limits _{0}^{\tau} \cos^2 \frac{-2\pi t}{\tau} + \mathrm{i} \sin^2 \frac{-2\pi t}{\tau} \mathrm{d}t
\end{align}
Notation: $\tau$ is the period of the frequency $\omega=\frac{1}{\tau}$, not a Turn $T$ from the Analyzer.\\
Note: The period $\tau$ is only relevant to the Fourier basis $e^{-\mathrm{i}t}$.\\\\
...
\begin{align}
\int\limits_{-\infty}^{\infty} \lvert \underbrace{y(\xi '(t))}_{\text{rotated to }xaxis}\rvert ^2 \mathrm{d} t = 1/2\pi * \int\limits_{-\infty}^{\infty} \vert \hat{f}(1/T)\circ \underbrace{\upsilon (\xi (t))}_{waveform} \vert ^2 \mathrm{d} \tau\\
(((\int \int,\norm{z}=\norm{r}_2),\odot),\norm{\phi}= \sup \phi)
\end{align}
