...
This can happen by summing the vectors in the Vector Graph up to the period $2^j$.
\begin{align}
(X,\norm{\cdot})\\
Z = \{a,b \in X \vert d(a,b)=(b-a)/2^j;a=c*(b-a);c \in \mathbb{Z}\}
\end{align}
Note: Downsampling the signal $s(t)$ before calculating the \emph{Vector Graph} instead, obliterates points in $\iota_{X}$ which can not be recovered. This is because as a consequence of downsampling, the Vector Graph folds differently, rendering the points in the original full resolution invalid.\\\\
If $j=0$, then $T=b-a$. The resulting limits for isolation and visibility are zero and one respectively.\\\\
