...
It is smooth (~\cite[Riemann Integrable]{Widon}). Therefore a convolution can be applied \emph{after} integration.\\
...
\chapter{Visibility}
Visibility is computed by creating an auxiliary set of chart-wise elements
\begin{equation}
h: U_{m} \rightarrow X \rightarrow O
\end{equation}
Note: Because of Corner insertion (\cite{Stopeight}[3.20]), $card(U_{m}) \leq 3$.\\\\
This requires finding Centres of Cliffs by the arithmetic mean,\\
\begin{equation}
O=\{(x',y') \in \mathbb{R}^2 \vert \min_{U_{m}} \sqrt{(x-x')^2 + (y-y')^2} \}
\end{equation}
\begin{align}
visibility(a,b)=\int \limits _{a}^{b} \sqrt{(x-x')^2 + (y-y')^2} * (\cos(angle(t)) + \mathrm{i}\sin(angle(t))) \mathrm{d}t\\
angle: t \mapsto \phi
\end{align}
Note: Integration \emph{after} convolution.\\\\
Finding their Handrolls (chart on Spiral, tangential space),\\
\begin{align}
C_{O}=\inf_{t \in U} \lvert \frac{\mathrm{d}}{\mathrm{d}t}visibility(t) \rvert \approx 0\\
\end{align}
Finding Eliptic Joints,\\
\begin{align}
T_{O}=\inf_{t \in V} \lvert \frac{\mathrm{d}^2}{\mathrm{d}t^2}visibility(t) \rvert \approx 0\\
\end{align}
Intersecting the normal in the Handroll with the line connecting the Joints. (or half diameter?)\\
