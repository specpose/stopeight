...
If we have $T_{3}$ and all the Compact Covers worked out, Korners can be redefined (Swell?) and new differences and areas parametrised. $p$ supremums are considered over the whole section of the Compact Cover $V_{n}$. The interval over the bounds of a supremum is $[a(K_{p}),b(K_{p})]$.
\begin{align}
...
\end{equation}
(horizontals on bisections? change of variable?)
\begin{align}
...
\begin{align}
area_{jitter}(a,b)=\frac{\int \limits _{0}^{(b-a)/2} \lvert \iota_{X}(b-t)-\iota_{X}(a+t)\rvert \mathrm{d}t}{\int \limits _{0}^{(b-a)/2} diameter_{abs}(a+t,b-t)\mathrm{d}t}\\
\end{align}
...
\begin{align}
visibility(a,b)= alt(a,b) * signs(\mu,solidangle_{Abs}(a,b))\\
...
\begin{align}
jaggedness(a,b)=\int \limits _{a}^{b} \lvert \iota_{X}(t)-\xi_{K}(t)\rvert \mathrm{d}t
\end{align}
...
Also Vector Graph manipulations such as scaling, {vert,, finding/counting intersections, oriented section occurence requirements could yield interesting results.\\\\
