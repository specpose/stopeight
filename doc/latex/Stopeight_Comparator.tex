...
\begin{align}
\frac{\log isolation(t)}{\log \overline{SE}} visibility(t)\\
\phi_{visibility} = 2\pi\frac{\overline{SE}}{2}t/(bs)
\end{align}
...
\section{Complex Visibility}
The polar mode is
\begin{align}
visibility_{bs}(a,b)=\int \limits _{a}^{b} \lvert \upsilon (\xi (t))\rvert \mathrm{e}^{\mathrm{i}\phi} \mathrm{d}\phi\\
\phi = \int \limits _{-\infty}^{\infty} \lvert \xi (t) \frac{\mathrm{d}}{\mathrm{d}t} \rvert \mathrm{d}t / (\tau = bs); bs \in (-\infty,\infty)\\
visibility_{bs} \frac{\mathrm{d}}{\mathrm{d}bs}\\
(((\int,\norm{z}=\norm{r}_1),\odot),\norm{\phi}= \norm{t}_2)\Rightarrow \norm{r}_1 \sim \norm{\phi}
\end{align}
