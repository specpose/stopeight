...
\begin{align}
orthogonal(a,b)=\int \limits _{-\infty}^{\infty} \lvert (x'+\mathrm{i}y')\rvert \mathrm{e}^{\mathrm{i}\phi} \mathrm{d}\phi\\
(\int,\norm{\phi}= \arctan \frac{y'}{x'})
\end{align}
Notation: There is absolutely no difference in infinitesimal $\int \limits _{a}^{b}$ notation and discrete $\sum  \limits _{i=a}^{b}$ notation if a metric space $(\mathbb{R},\norm{i}_2)$ is defined. A set $\mathbb{R}$ with bounds $a,b \in \mathbb{R}$ has to be open, closed $open \lor closed$ or clopen $open \land closed$.\\\\
The cartesian mode is also useful for simulation. Forward and backward $dir_{Y}$ signals can be added together.
\begin{align}
nonorthogonal(a,b)=\int \limits _{z(a)}^{z(b)} (x''+\mathrm{i}y'') \mathrm{d} t\\
(\int,\norm{t}=\norm{z''}_1)
\end{align}
...
\begin{align}
z_{n} = \int \limits _{t_{1}}^{t_{2}} \sin \underbrace{\frac{2\pi}{(t_{2}-t_{1})}t}_{\approx \text{numerator }Max_{Straight}} * \underbrace{\lvert (x'(t)+\mathrm{i}y'(t)) \rvert}_{1\text{ in this case}} \mathrm{d}t \label{eq:4}
\end{align}
Note: The resulting $z_{n}$ is entirely real.\\\\
Next create a $phaseprofile_{t_{2}-t_{1}}$ with $z_{n}$ derived from that waveform.
\begin{align}
z_{n} = \int \limits _{t_{1}}^{t_{2}} \lvert \underbrace{waveform_{1}'(x(t),y(t))}_{\text{orthogonal c.t.}} \rvert \mathrm{e}^{-2\pi t/(t_{2}-t_{1})}\mathrm{d}t
\end{align}
...
\begin{align}
\hat{f}(1/\tau) = \int \limits _{-\infty}^{\infty} \lvert \underbrace{waveform_{1}'(x(t),y(t))}_{\text{orthogonal c.t.}} \rvert \mathrm{e}^{-2\pi t/\tau}\mathrm{d}t
\end{align}
...
Note: A norm is a constraint on a space affecting a prospective differential. A base is an affine or non-affine transform mapping the prospective integrand. A norm for a frequency is a polar norm. A base for signal energy has a cartesian norm and is composed of the sum of polar bases.
