...
For a Lie Group
\begin{align}
\text{\{GL(3,$\mathbb{Z}$)} \subset M \vert \forall M \exists c_{1}\}
\end{align}
Axiom: For a spline base with $h_{1}=\begin{pmatrix}0 \\ 0 \\ 0\end{pmatrix}$ and $h_{2}=\begin{pmatrix}1 \\ 0 \\ 0\end{pmatrix}$, there exist only two Eigenvalues
\begin{align}
ev_{1}=Eigenvalue(
\underbrace{\begin{pmatrix}
a & 0 & 0\\
...
\end{pmatrix}}_{Base})
\end{align}
Theorem: The approximation algorithm imposes a constraint on the Eigenvectors
